...
#
# Inputs
# input_signal :  [Nx1]	Speech signal frame
# (n_vocal_tract)	:  [1x1]	Order of LP analysis for vocal tract (def. 48)
# (n_glottis)	:  [1x1]	Order of LP analysis for glottal source (def. 3)
...
#  [av,ag,al] = gfmiaif(x,nv,n_glottis,d,win) allows to choose parameters
#
...
# Thus, the use of n_glottis = 3 is highly encouraged.
#
#
# References
...
# function [av,ag,al] = gfmiaif(input_signal,n_vocal_tract,n_glottis,d,win)

# ----- Set default parameters -------------------------------------------
def gfmiaif(input_signal,n_vocal_tract=48,n_glottis=3,d=0.99,win="hann"):

    # Window for LPC estimation
    # TODO  win = hann(length(input_signal));
    ...
    Lpf = n_vocal_tract+1 # Pre-frame length
    expanded_signal = np.arange(-input_signal[0],input_signal[0]) 
    [linspace(-input_signal(1),input_signal(1),Lpf)' ; input_signal];    # Prepend
    idx_pf = (Lpf+1):length(expanded_signal);     # Indexes that exclude the pre-frame
    ...
    s_gv = filter(1,al,input_signal);
    # - Pre-framed input signal (for LPC envelope removal)
    x_gv = filter(1,al,expanded_signal);


    # ----- Gross glottis estimation -----------------------------------------

    # Iterative estimation of glottis with n_glottis first order filters
    ...
    av1 = lpc(s_v1.*win,n_vocal_tract);        # n_vocal_tract order LPC estimation
    ...
    ag = lpc(s_g1.*win,n_glottis);         # n_glottis order estimation
    ...
    av = lpc(s_v.*win,n_vocal_tract);          # n_vocal_tract order LPC estimation
